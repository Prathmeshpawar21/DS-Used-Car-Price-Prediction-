...
def homepage():
    ...
    test_columns = [
        'year', 'km_driven', 'mileage', 'engine', 'seats', 'name_Ambassador',
    ...
    encodedOHEDf = pd.DataFrame(encodedOHEArr,columns= ohe.get_feature_names_out(categorical_columns))
    ...
    encodedOHEDf.drop(columns=['year','km_driven','mileage','engine','seats'],inplace=True)
    ...
    records = [float(fromDataDict["year"]), float(fromDataDict["km_driven"]), float(fromDataDict["mileage"]), float(fromDataDict["engine"]), float(fromDataDict["seats"]) ]
    
    means = [float(2013.61114), float(73398.3377), float(19.4665848), float(1430.98586),float(5.43427125)]
    stds = [float(3.89711144), float(58698.9054), float(4.04780053), float(493.432463), float(0.983731806)]
    # Example inputted form data (ensure the order matches training data columns)
    # year, km_driven, engine, mileage, seats
    print(records)
    # Scale the form data
    scaled_form_data = [(value - mean) / std for value, mean, std in zip(records, means, stds)]
